#shellspec shell=/bin/sh

Describe 'jwk-to-pem'
    Include ./bin/jwk-to-pem

    It 'parses RSA private keys from JWK to PEM'
	When call main ./res/test/example-jwk-rsa-private-key.json
	The stderr should equal 'writing RSA key'
	The output should equal '-----BEGIN RSA PRIVATE KEY-----

fDueVy0nt0j5kXysGSOslNuGcb0ChWCLXZXVChszuiGus0yoQFUV
-----END RSA PRIVATE KEY-----'
    End
End
